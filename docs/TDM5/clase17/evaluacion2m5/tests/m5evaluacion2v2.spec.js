...
describe('Pruebas relator ComponenteAlternativa2.vue', () => {
  ...
  test('El componente recibe un parámetro x con un nombre, lo presenta en pantalla en un tag H1 donde dice "Hola x, bienvenido"', () => {
    ...
    const wrapper1 = mount(ComponenteAlternativa2, { localVue, propsData: { x: "Rodrigo" } });
  ...
  it('Los parámetros en pantalla se llenan y el botón ejecuta la suma', async () => {

    const wrapper = mount(ComponenteAlternativa2, { localVue })
    const input1 = wrapper.find('input.a');
    const input2 = wrapper.find('input.b');
    ...
    expect(wrapper.find("h2").html()).toContain("8");
